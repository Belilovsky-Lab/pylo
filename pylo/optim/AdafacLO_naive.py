...
def update_factors(
    v_col, v_row, v_full, g, g_shape, decay_rate: float = 0.9, epsilon: float = 1e-9
):
...
class AdafacLO_naive(Optimizer):
    ...
    def __init__(
        self,
        params,
        momentum_decays=torch.tensor([0.1522, 0.1425, 0.0681]),
        rms_decays=torch.tensor([0.0108]),
        adafactor_decays=torch.tensor([0.1862, -0.1086, -0.0619]),
        lr=1e-3,
        exp_mult=0.001,
        step_mult=0.001,
        input_size=39,
        ...
        clip_mom_decays = torch.clip(mom_decay.clone().detach(), 0.0, 1.0).to(self.device)
        clip_rms_decays = torch.clip(rms_decays.clone().detach(), 0.0, 1.0).to(self.device)
        clip_adafactor_decays = torch.clip(adafactor_decays.clone().detach(), 0.0, 1.0).to(
            self.device
        )
        ...
        if networks_type == "torch":
            #self.network = MetaMLP(input_size, hidden_size, hidden_layers).to(self.device)
            self.network = MetaMLP.from_pretrained("Pauljanson002/test").to(self.device)
        else:
            raise NotImplementedError("Only torch networks are supported now")
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                training_step_feature = tanh_embedding(group["step"]).to(self.device)
                ...
                if f_dims is not None:
                    ...
                    row_col_mean = fac_vec_row.mean(dim=d1, keepdim=True)
                    row_factor = torch.rsqrt(fac_vec_row / (row_col_mean + 1e-9))
                    col_factor = torch.rsqrt(fac_vec_col)
                    fac_mom_mult = (
